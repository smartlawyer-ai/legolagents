...
class TestLegalSource:
    def test_minimal_construction(self):
        src = LegalSource(ref="L1235-3", kind=SourceType.STATUTE, authority=Authority.BINDING)
        assert src.ref == "L1235-3"
        assert src.kind == SourceType.STATUTE
    ...
    def test_civil_law_vs_common_law_authority(self):
        ...
        civil_law_case = LegalSource(ref="21-14.027", kind=SourceType.CASE_LAW, authority=Authority.PERSUASIVE)
        common_law_case = LegalSource(ref="Roe v. Wade", kind=SourceType.CASE_LAW, authority=Authority.BINDING)
        assert civil_law_case.kind == common_law_case.kind
    ...
    def test_relates_to_with_enum(self):
        ...
        assert case.relations[0].kind == RelationType.INTERPRETS
    ...
    def test_chained_relations(self):
        old_case = LegalSource(ref="17-19.860", kind=SourceType.CASE_LAW, authority=Authority.PERSUASIVE)
        new_case = (
            LegalSource(ref="21-14.027", kind=SourceType.CASE_LAW, authority=Authority.PERSUASIVE)
            .relates_to(old_case, how="overturns")
        )
        assert len(new_case.relations) == 1
        assert new_case.relations[0].kind == RelationType.OVERTURNS

    def test_to_markdown_includes_authority_badge(self):
        statute = LegalSource(ref="L1235-3", kind=SourceType.STATUTE, authority=Authority.BINDING, title="Severance cap")
